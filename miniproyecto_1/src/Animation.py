...
output_dir = "miniproyecto_1/figures/event_disks_box_movie"
colors = ['r', 'b', 'g', 'orange']
...
def snapshot(t, pos, vel, colors, arrow_scale=0.2):
    ...
    pylab.text(0.5, 1.03, 't = %4.2f' % t, ha='center')
    pylab.savefig(os.path.join(output_dir, '%04d.png' % img))
    img += 1
...
sigma = 0.15
t = 0.0
dt = 0.02  # dt = 0.02 corresponde a animación paso a paso
n_steps = 100
...
os.system(
    "magick -delay 1 -dispose Background +page "
    + str(output_dir)
    + "/*.png -loop 0 "
    + str(output_dir)
    + "/animation.gif"

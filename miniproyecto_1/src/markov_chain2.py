...
# Cargar las configuraciones guardadas previamente
ruta_json = os.path.join("miniproyecto_1", "data", "configuraciones_ref_N8.json")

with open(ruta_json, "r", encoding="utf-8") as f:
    config_data = json.load(f)

# Convertir las listas leídas del JSON a tuplas de tuplas (inmutables para usar como claves)
conf_a = tuple(tuple(pos) for pos in config_data["conf_a"])
conf_b = tuple(tuple(pos) for pos in config_data["conf_b"])
conf_c = tuple(tuple(pos) for pos in config_data["conf_c"])
